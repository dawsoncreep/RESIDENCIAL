...
{
    public interface IAPIExternalUserService : IExternalService
    {

    }

    public class APIExternalUserService : IAPIExternalUserService
    {
        ...
        public ResponseHelper GetById(int id)
        {
            ...
            try
            {
                using (var ctx = _dbContextScopeFactory.CreateReadOnly())
                {
                    result = _externalRepo.SingleOrDefault(s => s.Id == id,
                        s => s.ExternalType,
                        l => l.Location);
                }
                rh.Result = result;
